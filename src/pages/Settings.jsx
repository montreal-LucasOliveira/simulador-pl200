...
export default function Settings() {
  ...
  const handleReset = () => {
    if (window.confirm("Deseja realmente resetar todas as preferências visuais?")) {
      setTheme('light');
      setFontSize('medium');
    }
  };
    ...
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-800'}`}>
      ...
      <nav className={`border-b sticky top-0 z-40 shadow-sm transition-colors ${theme === 'dark' ? 'bg-slate-900 border-slate-800 text-white' : 'bg-white border-slate-200 text-slate-800'}`}>
        <div className="max-w-5xl mx-auto px-4 h-16 flex justify-between items-center">
          ...
          <div className="font-black text-xl tracking-tighter">Configurações de UI</div>
      ...
      <main className="max-w-4xl mx-auto px-4 py-12">
        <header className="mb-12 text-center">
            <h1 className="text-4xl font-black mb-3">Personalize sua Experiência</h1>
            <p className={`font-medium ${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'}`}>Ajuste a interface para seu conforto máximo durante as horas de estudo.</p>
        </header>
        ...
        <div className="grid gap-8">
            ...
            <section className={`p-8 rounded-[3rem] border transition-all ${theme === 'dark' ? 'bg-slate-900 border-slate-800 shadow-2xl-blue' : 'bg-white border-slate-200 shadow-xl'}`}>
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h2 className="font-black text-xl">Aparência Visual</h2>
                        <p className={`text-sm font-bold ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>Escolha o esquema de cores da aplicação.</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    ...
                    >
                        ...
                        <span className="font-black text-sm uppercase tracking-widest text-slate-800">Modo Claro</span>
                    ...
                    >
                        ...
                        <span className={`font-black text-sm uppercase tracking-widest ${theme === 'dark' ? 'text-white' : 'text-slate-800'}`}>Modo Noturno</span>
            ...
            <section className={`p-8 rounded-[3rem] border transition-all ${theme === 'dark' ? 'bg-slate-900 border-slate-800 shadow-2xl-blue' : 'bg-white border-slate-200 shadow-xl'}`}>
                <div className="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h2 className="font-black text-xl">Acessibilidade e Texto</h2>
                        <p className={`text-sm font-bold ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>Ajuste o tamanho da fonte para uma leitura confortável.</p>
                    </div>
                </div>
                ...
                <div className={`mt-6 p-6 rounded-2xl border border-dashed text-center font-medium ${theme === 'dark' ? 'border-slate-700 text-slate-400' : 'border-slate-300 text-slate-600'}`}>
                    <p style={{ fontSize: fontSize === 'small' ? '0.8rem' : fontSize === 'large' ? '1.2rem' : '1rem' }}>
                        Visualização de teste: O Dataverse é um serviço de armazenamento altamente seguro para o Microsoft Power Platform.
                    </p>
                </div>
            </section>
            ...
            <section className="flex flex-col md:flex-row gap-4">
                ...
                >
                    <RefreshCw size={18} /> RESETAR TUDO
                </button>
            </section>
        </div>
      </main>
